...
from src.api.ogc_api.utils.ifc_filemanager import save_ifc_file_on_server
...
def execute_generate_tree_model(job_id: str, input_data: ModelParams):
    try:
        update_job_status(
            job_id,
        ...
        ifc_bytes = baum_modeller.create_trees(input_data)
...
def execute_generate_city_model(job_id: str, input_data: ModelParams):
    try:
        update_job_status(
            job_id,
            status=JobStatus.running,
        ...
        gml_files = HamburgOGCAPI.get_tiles(x1, y1, x2, y2)
        ...
        ifc_bytes = process_gml_to_ifc(
            gml_files=gml_files,
            project_name="Hamburg Buildings",
