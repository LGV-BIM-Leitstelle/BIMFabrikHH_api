...
class TestDGMModelGeneration:
    ...
    def test_successful_dgm_model_generation(
        ...
        with patch(
            ...
            assert mock_app.from_geotiffs.call_args.kwargs.get("guide_from_oaf") is None
            assert "model" in result
            assert result["model"]["filename"].startswith("DGM_")
            assert result["model"]["content_type"] == "application/x-step"

    def test_dgm_type_parcels_passes_guide_from_oaf(
        ...
        with patch(
            ...
            assert mock_app.from_geotiffs.call_args.kwargs["merge_parcels"] is True
    ...
    def test_dgm_rs_type_parcels_passes_guide_from_oaf(
        self, valid_dgm_request_params, sample_dgm_tiles
    ):
        payload = valid_dgm_request_params.model_dump()
        payload["dgm_type"] = 2
