...
class TestProcessExecution:
    ...
    def stub_admission_controller(self):
        """Stub out the Redis-backed admission controller.

        These endpoint tests construct the app without running its lifespan, so
        the rate limiter is skipped automatically. The concurrency controller,
        however, would otherwise reach out to Redis; here it is replaced with a
        stub that always grants capacity and records registrations.
        """
        with patch(
            "src.api.ogc_api.routes.main_ogc.get_admission_controller"
        ) as mock_get_controller:
            controller = Mock()
            controller.ensure_capacity.return_value = None
            controller.register_job.return_value = None
    ...
    def test_execute_nonexistent_process(self, client, valid_execution_input):
        ...
        response = client.post(
            "/ogc/processes/nonexistent-process/execution", json=valid_execution_input
        )
    ...
    def test_execute_with_invalid_input_missing_bbox(self, client):
        ...
        invalid_input = {"containers": []}

        response = client.post(
            "/ogc/processes/generate-tree-model/execution", json=invalid_input
        )
    ...
    def test_execute_with_invalid_input_wrong_type(self, client):
        ...
        invalid_input = {"bbox": "not_a_dict", "containers": []}
    ...
    def test_execute_with_empty_body(self, client):
        ...
        response = client.post("/ogc/processes/generate-tree-model/execution", json={})
...
class TestCORSHeaders:
    ...
    def test_cors_headers_on_get(self, client):
        ...
        response = client.get(
            "/ogc/processes", headers={"Origin": "http://localhost:3000"}
        )
        assert response.status_code == 200
...
class TestInputValidation:
    ...
    def test_invalid_bounding_box(self, client, invalid_bbox):
        ...
        invalid_input = {"bbox": invalid_bbox, "containers": []}
...
class TestContentNegotiation:
    ...
    def test_accept_json_header(self, client):
        ...
        response = client.get("/ogc/processes", headers={"Accept": "application/json"})
        assert response.status_code == 200
        assert "application/json" in response.headers["content-type"]
...
class TestEndToEndWorkflow:
    ...
    def test_complete_tree_model_workflow(
        ...
        # Stub admission control (no Redis in tests).
        mock_get_controller.return_value = Mock()
